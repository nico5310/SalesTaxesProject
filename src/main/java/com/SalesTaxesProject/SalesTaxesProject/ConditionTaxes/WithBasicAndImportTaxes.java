...
public class WithBasicAndImportTaxes implements CalculateTaxes {

    @Override
    public double calculate(Product product) {
        double priceWithTaxes = product.getPrice();
        priceWithTaxes += RulesFormat.roundingTaxesFormat(round(priceWithTaxes * (TaxPercent.BASIC_AND_IMPORT_TAX_PERCENT)) / 100.0);
